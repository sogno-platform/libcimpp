...
static const std::map<std::string, AttrDetails>& getClassAttrDetailsMap()
{
	...
	{
		{ "WindGenTurbineType2IEC.WindContRotorRIEC", { "http://iec.ch/TC57/2013/CIM-schema-cim16#", { CGMESProfile::DY, } } },
		{ "WindGenTurbineType2IEC.WindPitchContEmulIEC", { "http://iec.ch/TC57/2013/CIM-schema-cim16#", { CGMESProfile::DY, } } },
	};
...
{
	WindGenTurbineType2IEC* element = dynamic_cast<WindGenTurbineType2IEC*>(BaseClass_ptr1);
	WindPitchContEmulIEC* element2 = dynamic_cast<WindPitchContEmulIEC*>(BaseClass_ptr2);
	if (element != nullptr && element2 != nullptr)
	{
		if (element->WindPitchContEmulIEC != element2)
		{
			element->WindPitchContEmulIEC = element2;
...
bool get_WindGenTurbineType2IEC_WindPitchContEmulIEC(const BaseClass* BaseClass_ptr1, std::list<const BaseClass*>& BaseClass_list)
{
	const WindGenTurbineType2IEC* element = dynamic_cast<const WindGenTurbineType2IEC*>(BaseClass_ptr1);
	if (element != nullptr)
	{
		if (element->WindPitchContEmulIEC != 0)
		{
			BaseClass_list.push_back(element->WindPitchContEmulIEC);
			return true;
		}
	}
	return false;
...
void WindGenTurbineType2IEC::addClassAssignFnsToMap(std::unordered_map<std::string, class_assign_function>& assign_map)
{
	...
	assign_map.emplace("WindGenTurbineType2IEC.WindPitchContEmulIEC", &assign_WindGenTurbineType2IEC_WindPitchContEmulIEC);
}
...
void WindGenTurbineType2IEC::addClassGetFnsToMap(std::map<std::string, class_get_function>& get_map) const
{
	...
	get_map.emplace("WindGenTurbineType2IEC.WindPitchContEmulIEC", &get_WindGenTurbineType2IEC_WindPitchContEmulIEC);
}
