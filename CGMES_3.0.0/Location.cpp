...
static const std::map<std::string, AttrDetails>& getClassAttrDetailsMap()
{
	...
	{
		{ "Location.CoordinateSystem", { "http://iec.ch/TC57/CIM100#", { CGMESProfile::GL, } } },
		{ "Location.PositionPoints", { "http://iec.ch/TC57/CIM100#", { CGMESProfile::GL, } } },
...
bool assign_Location_CoordinateSystem(BaseClass* BaseClass_ptr1, BaseClass* BaseClass_ptr2)
{
	...
	if (element != nullptr && element2 != nullptr)
	{
		if (element->CoordinateSystem != element2)
		{
			...
			return assign_CoordinateSystem_Locations(BaseClass_ptr2, BaseClass_ptr1);
		}
		return true;
	}
	return false;
...
bool get_Location_mainAddress(const BaseClass* BaseClass_ptr1, std::list<const BaseClass*>& BaseClass_list)
{
	...
	if (element != nullptr)
	{
		...
		{
			BaseClass_list.push_back(element->mainAddress);
			return true;
		}
	}
	return false;
...
void Location::addClassAssignFnsToMap(std::unordered_map<std::string, class_assign_function>& assign_map)
{
	...
	assign_map.emplace("Location.mainAddress", &assign_Location_mainAddress);
}
...
void Location::addClassGetFnsToMap(std::map<std::string, class_get_function>& get_map) const
{
	...
	get_map.emplace("Location.mainAddress", &get_Location_mainAddress);
}
