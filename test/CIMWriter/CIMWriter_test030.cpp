...
BOOST_AUTO_TEST_CASE(CIMWriter_test030)
{
  ...
  BOOST_REQUIRE(model.addCIMFile("../../test/rdf/test030.xml"));
  model.parseFiles();
  ...
  BOOST_REQUIRE_EQUAL("_VL", model.Objects[2]->getRdfid());
  ...
  BOOST_REQUIRE_EQUAL("  <cim:PowerTransformer rdf:ID='_PT'>", lines[2]);
  BOOST_REQUIRE_EQUAL("    <cim:IdentifiedObject.name>PT</cim:IdentifiedObject.name>", lines[3]);
  BOOST_REQUIRE_EQUAL("    <cim:Equipment.EquipmentContainer rdf:resource='#_VL' />", lines[4]);
  BOOST_REQUIRE_EQUAL("  </cim:PowerTransformer>", lines[5]);
  BOOST_REQUIRE_EQUAL("  <cim:VoltageLevel rdf:ID='_VL'>", lines[6]);
