...
BOOST_AUTO_TEST_CASE(CIMWriter_test031)
{
  ...
  BOOST_REQUIRE(model.addCIMFile("../../test/rdf/test031.xml"));
  model.parseFiles();
  BOOST_REQUIRE_EQUAL(3, model.Objects.size());

  BOOST_REQUIRE_EQUAL("_PT", model.Objects[0]->getRdfid());
  BOOST_REQUIRE_EQUAL("_VL0", model.Objects[1]->getRdfid());
  ...
  BOOST_REQUIRE_EQUAL("  <cim:VoltageLevel rdf:ID='_VL0'>", lines[4]);
